...
class AuthController extends Controller
{
	...
	function postSignUp($req, $res)
	{
		...
		if ($validation->failed()) {
			return $res->withRedirect(
				$this->router->pathFor('auth.signup')
			);
		}
	...
	public function getSignOut($req, $res, $args) {
		...
		$this->container->flash->addMessage( 'info', 'Thanks for coming, We look forward to seeing you again' );
	...
	{
		$validation = $this->validator->validate($req, [
			'old_password' => v::notEmpty()->matchesPassword($this->container->auth->user()->password),
			'new_password' => v::stringType()->notEmpty()->noWhitespace(),
		]);
